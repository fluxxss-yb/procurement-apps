                ...
                for idx, item in enumerate(st.session_state['shopping_list']):
                    ...
                    with col[10]:    
                        with st.popover("📝"):
                            ...
                            unit_idx = units_option.index(item['Unit']) if item['Unit'] in units_option else 0
                            edit_unit = st.selectbox("Unit", units_option, index=unit_idx, key=f"eunit_{idx}")
...
# --- MENU: INPUT MASTER DATA ---
    ...
    if not df.empty:
        ...
        with col2:
            filtered_item = df[df['BRAND NAME'] == selected_brand]['ITEM NAME'].dropna().unique()
            selected_item = st.selectbox("Pilih Item Name", sorted(filtered_item))

        with col3:
            filtered_type = df[(df['BRAND NAME'] == selected_brand) & 
                               (df['ITEM NAME'] == selected_item)]['TYPE'].dropna().unique()
