...
# --- KONFIGURASI GITHUB (MENGAMBIL DARI STREAMLIT SECRETS) ---
...
def update_file_to_github(file_path, content_str, commit_message):
  ...
  if not GITHUB_TOKEN or not REPO_NAME:
    st.warning(
        "⚠️ GITHUB_TOKEN atau REPO_NAME belum diatur di Streamlit Cloud"
        " Secrets. Perubahan hanya tersimpan lokal."
    )
    return False
  ...
  if df.empty:
    ...
    with st.expander("➕ Tambah Barang ke Daftar", expanded=True):
      ...
      if st.button("Tambah ke Daftar Belanja"):
        ...
        if save_shopping_list(st.session_state["shopping_list"]):
